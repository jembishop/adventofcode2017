...
solution :: [Int] -> Int
solution l = fst $ findDupAndRem $ snd $ findDupAndRem $ iterate redistribute l
...
findDupAndRem :: Ord a => [a] -> (Int, [a])
findDupAndRem l = go 0 l S.empty
    where go i (x:xs) s = if S.member x s then (i, xs) else go (i+1) xs (S.insert x s)
